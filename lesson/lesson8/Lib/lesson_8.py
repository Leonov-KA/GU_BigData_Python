...
def div():
    try:
        user_1 = int(input('Введите делимое: '))
        user_2 = int(input('Введите делитель: '))
        if user_2 == 0:
...
class TypeCheck:
    ...
    def check_value(self):
        ...
        while True:
            try:
                try:
                    user_val = int(input('Укажите числа: '))
                    ex = input(f'Добавляем "{user_val}" в список. Продолжить? y/n: ').lower()
                ...
                ex = input(f"Указанно не число! Продолжить? y/n: ").lower()
                if ex == 'n':
                    print(self.my_list)
                    break
                else:
...
class OfficeEquipment:
    ...
    def income(self):
        try:
            name = input(f'Укажите наименование: ')
            price = int(input(f'Укажите цену за ед: '))
            quantity = int(input(f'Укажите количество: '))
...
"""
5) Реализовать проект «Операции с комплексными числами». Создайте класс «Комплексное число», реализуйте перегрузку 
